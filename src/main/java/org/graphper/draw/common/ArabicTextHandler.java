...
/**
 * Handles the processing of arabic text. In particular it determines the form each arabic char
 * should take. It also contains methods for substituting plain arabic glyphs with their shaped
 * forms. This is needed when the arabic text is rendered using an AWT font.
 *
...
public class ArabicTextHandler {
  ...
  /**
   * If the AttributedString contains any arabic chars, assigns an arabic form attribute,
   * i&#x2e;e&#x2e; initial|medial|terminal|isolated, to each arabic char.
   *
  ...
  public static AttributedString assignArabicForms(AttributedString as) {
    ...
    if (numChars >= 3) {
      ...
      char c = aci.next();
      int i = 1;
      for (char nextChar = aci.next(); nextChar != AttributedCharacterIterator.DONE;
    ...
    if (charOrder != null) {
      ...
      if (charOrder[0] != aci.getBeginIndex()) {
        ...
        Float x = (Float) aci.getAttribute(X);
        Float y = (Float) aci.getAttribute(Y);

        if (x != null && !x.isNaN()) {
          reorderedAS.addAttribute(X, new Float(Float.NaN), charOrder[0], charOrder[0] + 1);
          reorderedAS.addAttribute(X, x, 0, 1);
        }
        if (y != null && !y.isNaN()) {
          reorderedAS.addAttribute(Y, new Float(Float.NaN), charOrder[0], charOrder[0] + 1);
          reorderedAS.addAttribute(Y, y, 0, 1);
        }
      }
      as = reorderedAS;
    }
    ...
    for (int c = aci.first(); c != AttributedCharacterIterator.DONE; c = aci.next(), idx++) {
      if (StringUtils.isArabic((char) c)) {
        if (runStart == -1) {
          runStart = idx;
        }
      } else if (runStart != -1) {
  ...
  /**
   * Substitutes plain Arabic glyphs with their shaped forms based on context.
   * This is needed when Arabic text is rendered using an AWT font.
   * Simple Arabic ligatures will also be recognized and replaced by a single character,
   * so the length of the resulting string may be shorter than the original string.
   *
   * @param text The Arabic text to process.
   * @return A String containing the shaped versions of the Arabic characters.
   */
  public static String createSubstituteString(String text) {
    StringBuilder substString = new StringBuilder(text.length());
    ...
    for (int i = 0; i < text.length(); i++) {
      ...
      // Determine form based on the character's context
      int form = determineArabicForm(text, i);

      // Check for ligatures with the next character
      if (charStartsLigature(c) && (i + 1 < text.length())) {
        char nextChar = text.charAt(i + 1);
        int nextForm = determineArabicForm(text, i + 1);

        if (form == ARABIC_TERMINAL && nextForm == ARABIC_INITIAL) {
          int substChar = ArabicTextHandler.getSubstituteChar(c, nextChar, ARABIC_ISOLATED);
          if (substChar > -1) {
            substString.append((char) substChar);
            i++; // Skip next character since it's part of the ligature
            continue;
          }
        } else if (form == ARABIC_TERMINAL) {
        ...
        } else if (form == ARABIC_MEDIAL && nextForm == ARABIC_MEDIAL) {
      ...
      // If no ligature found, apply simple substitution based on form
      int substChar = getSubstituteChar(c, form);
      substString.append(substChar > -1 ? (char) substChar : c);
    }
  ...
  /**
   * Determines the Arabic form (isolated, initial, medial, terminal) of a character
   * in a word based on its context.
   *
   * @param text The Arabic text string.
   * @param index The index of the character within the text.
   * @return An integer representing the form of the Arabic character.
   */
  private static int determineArabicForm(String text, int index) {
    char previous = (index > 0) ? text.charAt(index - 1) : '\0';
    char next = (index < text.length() - 1) ? text.charAt(index + 1) : '\0';

    boolean prevIsArabic = arabicChar(previous);
    boolean nextIsArabic = arabicChar(next);

    if (!prevIsArabic && !nextIsArabic) {
      return ARABIC_ISOLATED;
    } else if (!prevIsArabic) {
      return ARABIC_INITIAL;
    } else if (!nextIsArabic) {
      return ARABIC_TERMINAL;
    } else {
      return ARABIC_MEDIAL;
    }
  }
      ...
      if (charStartsLigature(c) && (i+1 < end)) {
        ...
        if (form != null && nextForm != null) {
          if (form.equals(ARABIC_TERMINAL) && nextForm.equals(ARABIC_INITIAL)) {
            ...
            int substChar = getSubstituteChar
            ...
            int substChar = getSubstituteChar(c, nextChar,ARABIC_TERMINAL.intValue());
            if (substChar > -1) {
              substString.append((char)substChar);
              i++;
              continue;
  ...
  /**
   * Returns true if the character shapes to the right. Note that duel shaping characters also shape
   * to the right and so will return true.
   *
   * @param c The character to test.
  ...
  private static boolean arabicCharShapesRight(char c) {
    ...
    if ((charVal >= 0x0622 && charVal <= 0x0625) || (charVal == 0x0627) || (charVal == 0x0629) || (
        charVal >= 0x062F && charVal <= 0x0632) || (charVal == 0x0648) || (charVal >= 0x0671
        && charVal <= 0x0673) || (charVal >= 0x0675 && charVal <= 0x0677) || (charVal >= 0x0688
        && charVal <= 0x0699) || (charVal == 0x06C0) || (charVal >= 0x06C2 && charVal <= 0x06CB)
        || (charVal == 0x06CD) || (charVal == 0x06CF) || (charVal >= 0x06D2 && charVal <= 0x06D3)
  ...
  private static boolean arabicCharShapesDuel(char c) {
    ...
    if ((charVal == 0x0626) || (charVal == 0x0628) || (charVal >= 0x062A && charVal <= 0x062E) || (
        ...
        charVal == 0x6CE) || (charVal >= 0x06D0 && charVal <= 0x06D1) || (charVal >= 0x06FA
        && charVal <= 0x06FC)) {
      return true;
    }
    return false;
  ...
  /**
   * Returns true if character shapes to the left. Note that duel shaping characters also shape to
   * the left and so will return true.
   *
   * @param c The character to test.
  ...
  static int[][][] doubleCharRemappings = {
      ...
      {{0x0020, 0xFE70, -1, -1, -1},   // 0x064B
          {0x0640, -1, -1, -1, 0xFE71}}, {{0x0020, 0xFE72, -1, -1, -1}},  // 0x064C
      {{0x0020, 0xFE74, -1, -1, -1}},  // 0x064D
      {{0x0020, 0xFE76, -1, -1, -1},   // 0x064E
          {0x0640, -1, -1, -1, 0xFE77}}, {{0x0020, 0xFE78, -1, -1, -1},   // 0x064F
      {0x0640, -1, -1, -1, 0xFE79}}, {{0x0020, 0xFE7A, -1, -1, -1},   // 0x0650
      {0x0640, -1, -1, -1, 0xFE7B}}, {{0x0020, 0xFE7C, -1, -1, -1},   // 0x0651
      {0x0640, -1, -1, -1, 0xFE7D}}, {{0x0020, 0xFE7E, -1, -1, -1},   // 0x0652
      {0x0640, -1, -1, -1, 0xFE7F}}};
