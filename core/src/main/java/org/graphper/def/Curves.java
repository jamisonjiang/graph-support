...
public final class Curves {
   ...
   * @see <a
   *     href="https://github.com/erich666/GraphicsGems/blob/master/gems/FitCurves.c">FitCurves.c</a>
   */
   ...
   *     href="https://github.com/erich666/GraphicsGems/blob/master/gems/FitCurves.c">FitCurves.c</a>
   */
  public static <F extends FlatPoint> MultiBezierCurve fitCurves(
      List<F> points, FlatPoint leftTangent, FlatPoint rightTangent, double error) {
    ...
    return fitCurves(
        0, points.size() - 1, error, new MultiBezierCurve(), leftTangent, rightTangent, points);
  }

  private static <F extends FlatPoint> MultiBezierCurve fitCurves(
      int first,
      int last,
      double error,
    ...
    if (last - first + 1 == 2) {
      ...
      thirdOrderBezierCurve.v2 =
          Vectors.add(thirdOrderBezierCurve.v1, Vectors.scale(leftTangent, distance));
      thirdOrderBezierCurve.v3 =
          Vectors.add(thirdOrderBezierCurve.v4, Vectors.scale(rightTangent, distance));
    ...
    ThirdOrderBezierCurve bezierCurve =
        generateBezier(points, first, last, pointChordLens, leftTangent, rightTangent);
    ...
    fitCurves(splitIndex[0], last, error, curve, centerVector.reserve(), rightTangent, points);
    return curve;
  }
  ...
  public static FlatPoint besselEquationCalc(double t, FlatPoint... points) {
    Asserts.illegalArgument(
        points == null || points.length < 2, "points length can not be lower than 2");
    if (t == 0) {
      return points[0];
    }
   ...
   */
  public static ThirdOrderBezierCurve divideThirdBesselCurve(
      double t, boolean isFirstHalf, ThirdOrderBezierCurve bezierCurve) {
    Asserts.nullArgument(bezierCurve, "bezierCurve");
    FlatPoint dividePoint =
        besselEquationCalc(t, bezierCurve.v1, bezierCurve.v2, bezierCurve.v3, bezierCurve.v4);

    // Compute the point at time t on the v2 -> v3 vector
    FlatPoint intersection =
        Vectors.add(
            bezierCurve.v2, Vectors.multiple(Vectors.sub(bezierCurve.v3, bezierCurve.v2), t));
    ...
    if (isFirstHalf) {
      ...
      childCurve.v2 =
          Vectors.add(
              bezierCurve.v1, Vectors.multiple(Vectors.sub(bezierCurve.v2, bezierCurve.v1), t));
      childCurve.v3 =
          Vectors.add(childCurve.v2, Vectors.multiple(Vectors.sub(intersection, childCurve.v2), t));
      childCurve.v4 = dividePoint;
    } else {
      childCurve.v1 = dividePoint;
      childCurve.v3 =
          Vectors.add(
              Vectors.multiple(Vectors.sub(bezierCurve.v3, bezierCurve.v4), 1 - t), bezierCurve.v4);
      childCurve.v2 =
          Vectors.add(intersection, Vectors.multiple(Vectors.sub(childCurve.v3, intersection), t));
   ...
   */
  private static <F extends FlatPoint> ThirdOrderBezierCurve generateBezier(
      List<F> points,
      int first,
      int last,
   ...
   * Use least-squares method to find Bezier control points for region.
   */
  private static <F extends FlatPoint> double computeMaxError(
      List<F> points,
      int first,
      int last,
      ThirdOrderBezierCurve bezierCurve,
   ...
   */
  private static <F extends FlatPoint> double[] reparameterize(
      List<F> points,
      int first,
      int last,
      double[] pointChordLens,
    ...
    for (int i = first; i <= last; i++) {
      repl[i - first] =
          newtonRaphsonRootFind(bezierCurve, points.get(i), pointChordLens[i - first]);
    }
  ...
  private static double newtonRaphsonRootFind(ThirdOrderBezierCurve curve, FlatPoint p, double u) {
    ...
    for (int i = 0; i <= 1; i++) {
      q2[i] =
          new FlatPoint(
              (q1[i + 1].getX() - q1[i].getX()) * 2, (q1[i + 1].getY() - q1[i].getY()) * 2);
    }
  ...
  private static <F extends FlatPoint> double[] chordLengthParameterize(
      List<F> points, int first, int last) {
  ...
  private static <F extends FlatPoint> FlatPoint computeCenterTangent(
      List<F> points, int centerIndex) {
  ...
  /** A sequence of cubic Bezier curve segments. */
  ...
  public static class ThirdOrderBezierCurve implements Serializable {
    ...
    /** Creates a cubic Bezier curve from four non-null control points. */
    ...
    public void adjust(double v2AdjustRatio, double v3AdjustRatio) {
      this.v2 =
          Vectors.add(
              v1,
              Vectors.scale(
                  Vectors.sub(v2, v1), FlatPoint.twoFlatPointDistance(v2, v1) * v2AdjustRatio));
      this.v3 =
          Vectors.add(
              v4,
              Vectors.scale(
                  Vectors.sub(v3, v4), FlatPoint.twoFlatPointDistance(v3, v4) * v3AdjustRatio));
    }
