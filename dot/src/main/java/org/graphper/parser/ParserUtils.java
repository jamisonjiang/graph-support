...
public class ParserUtils {
  ...
  /** Collects DOT attributes by key, with later values replacing earlier ones. */
  ...
  private static void parseAttrs(
      DOTParser.A_listContext a_list, BiConsumer<String, String> pairConsumer) {
    if (a_list == null) {
      return;
    }
  ...
  public static void subgraphAttributes(
      DOTParser.Attr_listContext attr_list, Subgraph.SubgraphBuilder l) {
    Map<String, String> attrMap = getAttrMap(attr_list);
    attrMap.forEach((key, value) -> subgraphAttribute(key, value, l));
  }

  public static void subgraphAttributes(
      DOTParser.A_listContext a_list, Subgraph.SubgraphBuilder l) {
    parseAttrs(a_list, (key, value) -> subgraphAttribute(key, value, l));
  }

  /** Applies a supported DOT attribute to a subgraph builder. */
  ...
  public static void clusterAttributes(
      DOTParser.Attr_listContext attr_list, Cluster.ClusterBuilder l) {
    Map<String, String> attrMap = getAttrMap(attr_list);
  ...
  public static void graphAttributes(
      DOTParser.Attr_listContext attr_list, Graphviz.GraphvizBuilder gb) {
    Map<String, String> attrMap = getAttrMap(attr_list);
  ...
  /** Applies a supported DOT attribute to a graph builder. */
  ...
  /** Applies a supported DOT attribute to a cluster builder. */
  ...
  /** Applies DOT attributes to a node builder, interpreting labels according to the node shape. */
          ...
          switch (key.toLowerCase()) {
            ...
            case "label":
              labelHandle(
                  nodeBuilder::label,
                  nodeBuilder::table,
                  nodeBuilder::labelTag,
                  recordShape ? nodeBuilder::recordTag : null,
                  value);
              break;
            case "height":
  ...
  /** Applies DOT edge attributes, including endpoint labels, to a line builder. */
  ...
  /** Sets the tail or head cell and compass port from a DOT endpoint's port components. */
  ...
  /** Applies a supported HTML table attribute, ignoring missing targets, keys, or values. */
  ...
  /** Applies a supported HTML cell attribute, ignoring missing targets, keys, or values. */
  ...
  /** Applies a supported HTML font attribute, ignoring missing targets, keys, or values. */
  ...
  private static void setBoolean(Consumer<Boolean> boolConsumer, String val) {
    ...
    } else if ("false".equalsIgnoreCase(val) || "no".equalsIgnoreCase(val) || "0".equals(val)) {
  ...
  private static <T extends Enum<T>> void setEnum(
      Consumer<T> consumer, Class<T> enumClass, String name) {
  ...
  private static boolean isLabel(String key) {
    ...
    return "label".equalsIgnoreCase(key)
        || "taillabel".equalsIgnoreCase(key)
        || "headlabel".equalsIgnoreCase(key);
  }

  /**
   * Returns whether a {@code shape} attribute value denotes a record shape, for which {@code {} }
   * and {@code |} in an angle-bracket label carry structure rather than text.
   */
  ...
  private static void labelHandle(
      Consumer<String> labelConsumer,
  ...
  /**
   * Routes a parsed label to the matching text, table, rich-text, or record consumer.
   *
  ...
  private static <T extends Enum<T>> T[] enumValues(String value, Class<T> type) {
    ...
    T[] values =
        Stream.of(value.split(SvgConstants.COMMA))
            .map(String::trim)
